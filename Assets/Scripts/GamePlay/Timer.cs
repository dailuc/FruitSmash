...
public class Timer : MonoBehaviour
{
	...
	public GameObject recoveryPopUp;
    ...
    public void Win()
    {
		//if (FB.IsInitialized)
		//FB.LogAppEvent(
		//	"WinLevel"+PLayerInfo.MapPlayer.Level
		//);
    ...
    public void Lost()
    {
		//if (FB.IsInitialized)
		//FB.LogAppEvent(
		//	"LostLevel"+PLayerInfo.MapPlayer.Level
		//);
		int playerHeart=PlayerPrefs.GetInt(Constant.HEART,5);
		if (playerHeart==5) PlayerPrefs.SetString("lasttimeloseheart",System.DateTime.Now.ToString ());
		playerHeart--;
		PlayerPrefs.SetInt(Constant.HEART,playerHeart);
		PlayerPrefs.Save();
	...
	public void showPurchaseHeart(){
		Animation am = recoveryPopUp.GetComponent<Animation>();
		am.enabled = true;
		recoveryPopUp.SetActive(true);
	}
	public void closePurchaseheart(){
		SoundController.Sound.Click();
