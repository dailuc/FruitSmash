...
public class GameController : MonoBehaviour
{
    ...
    IEnumerator DestroyType(int type, Vector3 pos)
    {
        ...
        for (int x = 0; x < 7; x++)
        {
            for (int y = 0; y < 9; y++)
            {
                ...
                {
                    EffectSpawner.effect.MGE(pos, JewelSpawner.spawn.JewelGrib[x, y].transform.position,false);
                    tmp.Destroy();
    ...
    IEnumerator TimeInc()
    {
        ...
        while (t > 0)
        {
            ...
            if (Timer.timer.GameTime >= 300f)
            {
                Timer.timer.GameTime = 300f;
                break;
            }
            t -= 1;
            yield return null;
